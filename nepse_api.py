...
def csv_json(csv_file):
    ...
    df["PREVIOUS_DAY_CLOSE_PRICE"] = pd.to_numeric(
        df["PREVIOUS_DAY_CLOSE_PRICE"], errors="coerce"
    )
    df["MARKET_CAPITALIZATION"] = pd.to_numeric(
        df["MARKET_CAPITALIZATION"], errors="coerce"
    )
    df["TOTAL_TRADED_VALUE"] = pd.to_numeric(df["TOTAL_TRADED_VALUE"], errors="coerce")
    df["TOTAL_TRADED_QUANTITY"] = pd.to_numeric(
        df["TOTAL_TRADED_QUANTITY"], errors="coerce"
    )

    df["PERCENTAGE_CHANGE"] = (
        (df["CLOSE_PRICE"] - df["PREVIOUS_DAY_CLOSE_PRICE"])
        / df["PREVIOUS_DAY_CLOSE_PRICE"]
        * 100
    )
...
def display_details(stock):
    ...
    if stock["PERCENTAGE_CHANGE"] > 0:
        st.write(
            f"**Percentage Change:** <span style='color:green'>{stock['PERCENTAGE_CHANGE']}</span>",
            unsafe_allow_html=True,
        )
    else:
        st.write(
            f"**Percentage Change:** <span style='color:red'>{stock['PERCENTAGE_CHANGE']}</span>",
            unsafe_allow_html=True,
        )
...
def main():
    ...
    st.dataframe(
        df.drop(columns=["BUSINESS_DATE", "S.N", "SECURITY_ID"]).style.applymap(
    ...
    top_traded_quantity = df.sort_values(
        by="TOTAL_TRADED_QUANTITY", ascending=False
    ).head(10)
    top_traded_quantity = top_traded_quantity.drop(
        columns=["BUSINESS_DATE", "S.N", "SECURITY_ID"]
    )
    st.dataframe(top_traded_quantity)

    # sort by MARKET_CAPITALIZATION
    st.title("Top Market Capitalization")
    top_market_capitalization = df.sort_values(
        by="MARKET_CAPITALIZATION", ascending=False
    ).head(10)
    top_market_capitalization = top_market_capitalization.drop(
        columns=["BUSINESS_DATE", "S.N", "SECURITY_ID"]
    )
