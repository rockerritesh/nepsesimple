...
from utils.nepsesimple import csv_json, get_ipo_message, get_stock_details
...
def main():
    ...
    fig.add_trace(
        go.Scatter(
            x=df_plot["Date (AD)"],
            y=df_plot["Index Value"].rolling(window=5).mean(),
            mode="lines",
    ...
    st.dataframe(
        df
    )  # .style.applymap(lambda x: 'color: red' if x < 0 else 'color: green', subset=['PERCENTAGE_CHANGE'])
